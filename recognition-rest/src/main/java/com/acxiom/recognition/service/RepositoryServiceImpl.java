...
import com.acxiom.recognition.dao.RepositoryDao;

import com.acxiom.recognition.exception.ErrorConstants;
...
//        }
        ...
        return repositoryDao.createRepository(new RepositoryEntity(repository));
    }
...
//
    public Repository getRepositoryById(Long id) throws RecognitionAppException {
        RepositoryEntity repositoryById = repositoryDao.getRepositoryById(id);
        if (repositoryById == null) {
            throw new RecognitionAppException(Response.Status.NOT_FOUND.getStatusCode(),
                    404,
                    "The repository you requested with id " + id + " was not found in the database",
                    "Verify the existence of the repository with the id " + id + " in the database",
                    ErrorConstants.REPOSITORY_POST_URL);
        }
        return new Repository(repositoryDao.getRepositoryById(id));
    }
//
